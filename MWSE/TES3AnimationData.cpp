...
namespace TES3 {
	...
	const auto TES3_AnimationData_calcAnimRootMovement = reinterpret_cast<void(__thiscall*)(AnimationData*, unsigned char)>(0x46FD80);
	void AnimationData::calcAnimRootMovement(unsigned char animGroup) {
	...
	void AnimationData::playAnimationGroupForIndex(int animationGroup, int bodySection, int startFlag, int loopCount) {
		if (mwse::lua::event::PlayAnimationGroupEvent::getEventEnabled()) {
			auto stateHandle = mwse::lua::LuaManager::getInstance().getThreadSafeStateHandle();
			sol::object response = stateHandle.triggerEvent(new mwse::lua::event::PlayAnimationGroupEvent(this, animationGroup, bodySection, startFlag, loopCount));
